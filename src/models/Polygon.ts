  ...
  private convexHull (): void {
    ...
    for (let i = 1; i < this.vertexList.length; i++) {
      while (
        ...
        this.getOrientation(this.polarRef, this.vertexList[i], this.vertexList[i + 1]) === Orientation.COLLINEAR
      ) {
        i++
      }
    ...
    for (let i = 3; i < newArrLen; i++) {
      while (
        this.getOrientation(hull[hull.length - 2], hull[hull.length - 1], this.vertexList[i]) !== Orientation.COUNTER_CLOCKWISE
      ) {
        hull.pop()
      }
